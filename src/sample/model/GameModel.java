...
public final class GameModel implements GameModelView {
    ...
    private final Foundation aFoundations = new Foundation();
    private final Tableau aTableau = new Tableau();
    private final List<GameModelListener> aListeners = new ArrayList<>();
    private final PlayingAgent aPlayingAgent;

    /**
     * Creates a new game model initialized to a new game.
     * @param pPlayingAgent The strategy to use for auto-play.
     * @pre pPlayingStrategy != null
     */
    public GameModel(PlayingAgent pPlayingAgent)
    {
        assert pPlayingAgent != null;
        aPlayingAgent = pPlayingAgent;
        reset();
    }
    ...
    {
        Move move = aPlayingAgent.getLegalMove(this);
        move.perform();
        return !move.isNull();
    }
    ...
    private Location find(Card pCard)
    {
        ...
        {
            return DiscardLocation.DISCARD_PILE;
        }
    ...
    {
        if( pLocation == DiscardLocation.DISCARD_PILE )
        {
    ...
    private void move(Card pCard, Location pDestination)
    {
        ...
        else
        {
            ...
            else if( pDestination == DiscardLocation.DISCARD_PILE )
            {
                aDiscard.push(pCard);
            }
            else
